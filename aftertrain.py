...
def get_dsF1(p="",
             path="/content/BERT-QA/semparse/data/ours",
             data_type='entity'):
    """ Assumes wordpiece-level predictions for spans"""
    print(p)
    token_path=path+'/tokenmat.npy'
    relation_path=path+'/relations.npy'
    entity_path=path+'/entities.npy'
    ...
    if data_type=='relation':
      my_gold = y_test[:, 2:]
    my_tokmat = X_test
    my_unbert = np.zeros_like(my_tokmat)
    for i in range(len(my_unbert)):
        counter = 1
        for j in range(len(my_unbert[i])):
            if my_tokmat[i, j]!=0:
                my_unbert[i, j] = counter
                counter +=1
    borderpreds = torch.tensor(np.load(os.path.join(p, "borderpreds.test.npy")))
    #bordergold = torch.tensor(ioborders[devstart:teststart]).long()
    unbertmat = torch.tensor(my_unbert).long()
    tokmat = torch.tensor(my_tokmat).long()
    ...
    # print(borderprobs[:100])
    # print(unbertmat.unsqueeze(1).repeat(1, 2, 1).shape)
    # print(borderprobs.shape)
    # print(borderprobs[0], np.argmax(borderprobs[0], axis=1))
    borders = np.argmax(borderprobs, axis=2)
    # print(borders.shape)
    # word_border_probs.scatter_add_(2, unbertmat.unsqueeze(1).repeat(1, 2, 1), borderprobs)
    word_border_golds = torch.tensor(my_gold).long()#[devstart:teststart] #unbertmat.gather(1, bordergold)

    # pred_start, pred_end = torch.argmax(word_border_probs, 2).split(1, dim=1)
    # pred_start, pred_end = pred_start - 1, pred_end - 1
    pred_start = torch.from_numpy(np.expand_dims(borders[:, 0], axis=0).reshape(-1, 1))
    pred_end = torch.from_numpy(np.expand_dims(borders[:, 1], axis=0).reshape(-1, 1))
    gold_start, gold_end = word_border_golds.split(1, dim=1)
    # print(pred_start[:10], pred_end[:10], pred_start.shape)
    # print(gold_start[:10], gold_end[:10], gold_start.shape)
    ...
    result_data = {'pred_start':pred_start.tolist(),
                   'pred_end':pred_end.tolist(),
                   'gold_start':gold_start.tolist(),
                   'gold_end':gold_end.tolist()}
    pd.DataFrame(result_data).to_excel('relation.xlsx')
    # pred_start, pred_end = torch.argmax(word_border_probs, 2).split(1, dim=1)
    # pred_start, pred_end = pred_start - 1, pred_end - 1
    gold_start, gold_end = word_border_golds.split(1, dim=1)
    overlap_start = torch.max(pred_start, gold_start)
...
def get_dsF1_wordlevel(p="exp_bilstm_span_borders_12",     #p="exp_bert_both_23",
    ...
    print(type(wordborders), type(devstart), type(teststart))
    tokmat = data["wordmat"]
...
if __name__ == '__main__':
    print('results for entity recognation:')
    get_dsF1(p="/content/BERT-QA/semparse/semparse/simplequestions/exp_bert_span_borders_0", 
             data_type='entity')
    print('\n\n\nresults for relation recognation:')
    get_dsF1(p="/content/BERT-QA/semparse/semparse/simplequestions/exp_bert_span_borders_1", 
          data_type='relation')
...
import random
from sq_relation_dataset import ReverbDataset
...
train, dev, test = ReverbDataset.splits(TEXT, RELATION, args.data_dir)
...
if args.dataset == 'RelationPrediction':
    print(n_dev_correct)
    P = 1. * n_dev_correct / len(test)
    print("{} Precision: {:10.6f}%".format("Test", 100. * P))
else:
    print("Wrong dataset")
    exit()

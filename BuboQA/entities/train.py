...
from sq_entity_dataset import SQdataset, ReverbDataset
...
TEXT = data.Field(fix_length=15, lower=True)
ED = data.Field(fix_length=15)

train, dev, test = ReverbDataset.splits(TEXT, ED, args.data_dir)
        ...
        if iterations % args.dev_every == 0:
            ...
            for dev_batch_idx, dev_batch in enumerate(dev_iter):
                ...
                if args.dataset == 'EntityDetection':
                    ...
                    # print(torch.max(answer, 1)[1].view(dev_batch.ed.size()).cpu().data.tolist())
                    temp = np.array(torch.max(answer, 1)[1].view(dev_batch.ed.size()).cpu().data.tolist())
                    index_tag = np.transpose(temp)
                    temp = np.array(np.transpose(dev_batch.ed.cpu().data.tolist()))
...
if args.dataset == 'EntityDetection':
  P, R, F = evaluation(gold_list, pred_list, index2tag, type=False)
  print("{} Precision: {:10.6f}% Recall: {:10.6f}% F1 Score: {:10.6f}%".format("Test", 100. * P, 100. * R,
                                                                                         100. * F))